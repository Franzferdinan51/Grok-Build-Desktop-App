...
- **Provider diagnostics and expansion** — add any number of named OpenAI-compatible providers, test their `/models` endpoint without exposing credentials, and remove custom entries without touching hand-written Grok configuration.
...
- **Daily-driver controls** — searchable skills and run history, visible Grok session/model/error metadata, run-now scheduling, keyboard task launch, cancellation, and refresh controls.
