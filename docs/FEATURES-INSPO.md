...
| [Franzferdinan51/Duck-Agent](https://github.com/Franzferdinan51/Duck-Agent) | Grok Build-primary CLI plus a Hermes-derived desktop design system (flat surfaces, tokens, chat-as-home, overlay elevation, compact file/review trees, page-search chrome, master-detail lists, session-scoped rails) | `/workflow`, `/doctor`, shared CSS tokens, status bar, command-palette elevation, chat-side Project Files / Task Inspector / terminal rails, compact workspace/review trees, page chrome, master-detail Skills/Scheduled/Runs, and a pairing/reconnect Telegram workbench | Replacing this Solid/Electron workbench with the Duck-Agent React/Hermes shell, Duck Agent branding, or a second agent runtime |
...
- **Workspace attachments:** a Duck-Agent-inspired composer flow lets users attach up to eight files already visible in the safe workspace file rail. The renderer sends relative workspace paths as prompt context; it does not expose arbitrary outside-workspace paths or add another execution harness.
