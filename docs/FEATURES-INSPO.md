...
| [Franzferdinan51/Duck-Agent](https://github.com/Franzferdinan51/Duck-Agent) | Grok Build-primary CLI with plan/research/code/operate workflows and a doctor check | `/workflow` and `/doctor` map those governed workflows onto the existing `grok -p --output-format streaming-json` path | Duck-Agent Python launcher, Prime/Hermes backends, or a second Electron agent runtime |
