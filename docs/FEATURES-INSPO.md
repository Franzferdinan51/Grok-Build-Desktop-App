...
| [Franzferdinan51/Duck-Agent](https://github.com/Franzferdinan51/Duck-Agent) | Grok Build-primary CLI plus a Hermes-derived desktop design system (flat surfaces, tokens, chat-as-home, overlay elevation, a session-scoped project file tree, compact live activity/status rows, and transient notifications) | `/workflow`, `/doctor`, shared CSS tokens, status bar, command-palette elevation, the chat-side expandable Project Files tree, a read-only Task Inspector backed by the live Grok event stream, and lifecycle toasts for started, queued, completed, and failed tasks | Replacing this Solid/Electron workbench with the Duck-Agent React/Hermes shell, Duck Agent branding, or a second agent runtime |
