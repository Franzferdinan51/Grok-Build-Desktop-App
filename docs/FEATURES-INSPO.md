...
- **Settings and sign-in:** everyday defaults stay CLI-fast (reasoning, self-verify, and subagents off; permission mode auto). Settings is Essentials / Accounts / Advanced. xAI OAuth is `grok login --oauth` with a signed-in probe that never returns tokens.
