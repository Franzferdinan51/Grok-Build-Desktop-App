...
| [Franzferdinan51/Duck-Agent](https://github.com/Franzferdinan51/Duck-Agent) | Grok Build-primary CLI plus a Hermes-derived desktop design system (flat surfaces, tokens, chat-as-home, overlay elevation, a session-scoped project file tree, and compact live activity/status rows) | `/workflow`, `/doctor`, shared CSS tokens, status bar, command-palette elevation, the chat-side expandable Project Files tree, and a read-only Task Inspector backed by the live Grok event stream | Replacing this Solid/Electron workbench with the Duck-Agent React/Hermes shell, Duck Agent branding, or a second agent runtime |
