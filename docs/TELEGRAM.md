...
3. Pause polling keeps the encrypted token so **Reconnect** works without pasting again. **Remove token** is the hard forget. Reconnect stops the in-flight `getUpdates` and waits briefly so a second poller does not 409 itself. Bootstrap classifies `deleteWebhook` the same way as `getUpdates` (pause on 401/409, back off on 429) and the UI is not live until that succeeds. A 429/409 on connect is not treated as a revoked token.
