...
test("desktop update state moves from available through download to ready", async () => {
  const state = await loadStateModule()
  const initial = state.initialDesktopUpdateState("0.2.37")
  const available = state.updateAvailableState(initial, {
    version: "0.2.38",
...
test("desktop update errors preserve version context and expose a safe message", async () => {
  ...
  const failed = state.updateErrorState(initial, new Error("network unavailable"))
