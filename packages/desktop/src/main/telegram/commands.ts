...
function prefixToKind(prefix: typeof TELEGRAM_CALLBACK_PREFIXES[number]): TelegramCallbackKind {
  ...
  if (prefix === "approve_task") return "approve_task"
  if (prefix === "deny_task") return "deny_task"
  return "menu"
}

export const TELEGRAM_HELP_TEXT = "**Grok Build Desktop Agent**\n\n/run <task> — run an agent task\n/learn [request] — draft a reusable skill from this task or recent work\n/goal [status|start|edit|pause|resume|complete|block|clear] — manage the persistent goal\n/commands — show this command list\n/new — start a fresh session\n/status — detailed agent status\n/security — NemoClaw policy and audit status\n/security on|off — toggle security mode\n/security approvals on|off — toggle approval gate\n/approve or /deny — resolve a held sensitive task\n/models — choose a model or configure MoA\n/moa — configure Mixture of Agents\n/project — choose Project, Scratch, or Agent mode\n/mode [fast|balanced|deep] — response-speed profile\n/queue — show queued work\n/steer <task> — prioritize the next instruction\n/interrupt <task> — stop and redirect active work\n/retry — retry the previous instruction\n/undo — rewind the previous turn\n/compress — checkpoint and compact context\n/reasoning [on|off] — session reasoning control\n/history — show recent visible conversation\n/schedules — scheduled agent work\n/sethome — deliver scheduled results to this chat\n/home — show the scheduled home channel\n/tools — show available agent surfaces\n/diagnostics — show backend and local bridge diagnostics\n/login <xai|openai|minimax> — start provider login\n/whoami — show this Telegram chat id\n/skills — list loaded skills\n/workspace — show the active workspace\n\nGrok Build compatibility commands:\n/skill /tasks /context /btw /side /session /subagents /acp /focus /unfocus /agents /config /mcp /plugins /debug /usage /stop /reset /name /compact /think /verbose /trace /fast /elevated /exec /model /activation /send /allowlist /bash\n\nCommands that require the OpenClaw Gateway or a different runtime return a clear support message here. Plain messages continue the current agent session."
