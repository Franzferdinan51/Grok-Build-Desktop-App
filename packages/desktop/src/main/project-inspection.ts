...
const gitOptions = (cwd: string) => ({ cwd, timeout: 4_000, killSignal: "SIGKILL" as const, maxBuffer: 2_000_000 })
...
export async function inspectProject(project: ProjectRecord): Promise<ProjectSnapshot> {
  ...
  try {
    ...
    const [{ stdout: branch }, { stdout: porcelain }, { stdout: diffStat }] = await Promise.all([
      run("git", ["branch", "--show-current"], gitOptions(cwd)),
      run("git", ["status", "--porcelain", "--untracked-files=normal"], gitOptions(cwd)),
      run("git", ["diff", "--stat"], gitOptions(cwd)),
    ])
