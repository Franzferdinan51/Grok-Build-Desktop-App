...
export type StoredChatThread = { id: string; workspace: string; title: string; createdAt: number; updatedAt: number; messages: { id: string; role: "user" | "assistant"; logs: { kind: "text" | "thought" | "activity" | "error"; content: string }[]; createdAt: number }[]; sessionId: string; model?: string; summary?: string; pinned?: boolean; archived?: boolean; sessionStatus?: "new" | "resumable" | "recovered" | "broken" }
...
export type ElectronAPI = {
  ...
  workspace: { files: (root: string) => Promise<WorkspaceFile[]>; read: (root: string, path: string) => Promise<string>; write: (root: string, path: string, content: string) => Promise<void>; command: (root: string, command: string) => Promise<{ stdout: string; stderr: string; code: number }>; gitChanges: (root: string) => Promise<{ status: string; path: string; staged?: boolean }[]>; gitDiff: (root: string, path: string) => Promise<string>; gitAction: (root: string, path: string, action: "stage" | "unstage" | "discard") => Promise<void>; gitWorktrees: (root: string) => Promise<GitWorktree[]>; gitBranches: (root: string) => Promise<GitBranch[]>; createGitWorktree: (root: string, input: { name: string; branch: string; mode?: "new" | "existing"; baseRef?: string }) => Promise<GitWorktree>; pathForFile: (file: { path?: string }) => string }
...
const api: ElectronAPI = {
  ...
  workspace: { files: (root) => ipcRenderer.invoke("workspace:files", root), read: (root, path) => ipcRenderer.invoke("workspace:read", root, path), write: (root, path, content) => ipcRenderer.invoke("workspace:write", root, path, content), command: (root, command) => ipcRenderer.invoke("workspace:command", root, command), gitChanges: (root) => ipcRenderer.invoke("workspace:git-changes", root), gitDiff: (root, path) => ipcRenderer.invoke("workspace:git-diff", root, path), gitAction: (root, path, action) => ipcRenderer.invoke("workspace:git-action", root, path, action), gitWorktrees: (root) => ipcRenderer.invoke("workspace:git-worktrees", root), gitBranches: (root) => ipcRenderer.invoke("workspace:git-branches", root), createGitWorktree: (root, input) => ipcRenderer.invoke("workspace:create-git-worktree", root, input), pathForFile: (file) => {
