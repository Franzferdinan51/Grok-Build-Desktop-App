...
export type ElectronAPI = {
  ...
  workspace: { files: (root: string) => Promise<WorkspaceFile[]>; read: (root: string, path: string) => Promise<string>; write: (root: string, path: string, content: string) => Promise<void>; command: (root: string, command: string) => Promise<{ stdout: string; stderr: string; code: number }>; gitChanges: (root: string) => Promise<{ status: string; path: string; staged?: boolean }[]>; gitDiff: (root: string, path: string) => Promise<string>; gitAction: (root: string, path: string, action: "stage" | "unstage" | "discard") => Promise<void> }
