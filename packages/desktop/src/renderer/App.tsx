    ...
    <main class={`main-content ${active() === "new-task" && previewEnabled() && previewOpen() ? "main-content--preview" : ""}`}>
      ...
      <Show when={active() === "telegram"} fallback={
        ...
        <section class="runs-panel"><span class="eyebrow">GROK BUILD SETTINGS</span><h1>Backend, models, and providers.</h1><p>The maintained backend is your <button class="link-button" onClick={() => window.api.app.openExternal("https://github.com/Franzferdinan51/grok-build")}>Franzferdinan51/grok-build fork</button>, with xAI upstream sync preserved. Every provider remains a Grok Build model target.</p>
          ...
          <div class="settings-card"><div><strong>Provider sign-in</strong><span>Use provider-native authentication when officially supported.</span></div><div class="provider-fields"><div><strong>xAI / Grok</strong><span>OAuth through the installed Grok CLI</span></div><button class="primary" onClick={async () => { try { const result = await window.api.backend.oauthLogin("xai"); setOauthNotice(result.message) } catch (error) { setOauthNotice(error instanceof Error ? error.message : String(error)) } }}>Sign in with xAI</button><div><strong>OpenAI</strong><span>API key required for third-party OpenAI-compatible clients</span></div><button disabled title="OpenAI does not offer a general third-party ChatGPT OAuth grant">OAuth unavailable</button><div><strong>MiniMax</strong><span>API key required by the public OpenAI-compatible endpoint</span></div><button disabled title="MiniMax has no documented public OAuth grant for this endpoint">OAuth unavailable</button></div><Show when={oauthNotice()}><p class="provider-notice">{oauthNotice()}</p></Show><p class="provider-notice">OpenAI ChatGPT/Codex subscription OAuth and MiniMax Portal login tokens are product-specific and are not copied or impersonated. API keys remain encrypted through the provider cards below.</p></div>
