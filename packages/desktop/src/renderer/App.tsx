    ...
    <main class={`main-content ${active() === "new-task" ? "main-content--chat" : ""} ${active() === "new-task" && previewEnabled() && previewOpen() ? "main-content--preview" : ""}`}>
      ...
      <Show when={active() === "telegram"} fallback={
        ...
        <section class="runs-panel"><div class="settings-brand"><img src={grokBuildLogo} alt="Grok Build Desktop logo" /><div><span class="eyebrow">GROK BUILD SETTINGS</span><h1>Backend, models, and providers.</h1><p>Grok Build Desktop · local-first agentic coding powered by the Grok Build CLI.</p></div></div><p>The maintained backend is your <button class="link-button" onClick={() => window.api.app.openExternal("https://github.com/Franzferdinan51/grok-build")}>Franzferdinan51/grok-build fork</button>, with xAI upstream sync preserved. Every provider remains a Grok Build model target.</p>
          ...
          <div class="settings-card"><div><strong>Provider sign-in</strong><span>Official provider OAuth plus Hermes-managed OpenAI Codex OAuth.</span></div><div class="provider-fields"><div><strong>xAI / Grok</strong><span>Official OAuth through the installed Grok CLI</span></div><button class="primary" onClick={() => void signInProvider("xai")}>Sign in with xAI</button><div><strong>OpenAI Codex</strong><span>Hermes OAuth using a ChatGPT/Codex subscription</span></div><button onClick={() => void signInProvider("openai")}>Sign in with OpenAI</button><div><strong>MiniMax</strong><span>Official MiniMax CLI device OAuth with PKCE and automatic refresh</span></div><button onClick={() => void signInProvider("minimax")}>Sign in with MiniMax</button></div><Show when={oauthNotice()}><p class="provider-notice">{oauthNotice()}</p></Show><p class="provider-notice">xAI OAuth feeds Grok Build directly. MiniMax credentials are owned and refreshed by the official <code>mmx</code> CLI; OpenAI Codex credentials are managed by Hermes and routed through a token-isolated local bridge. Available Codex models are imported automatically after sign-in.</p></div>
