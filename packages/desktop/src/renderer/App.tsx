    ...
    <main class={`main-content ${active() === "new-task" ? "main-content--chat" : ""} ${active() === "new-task" && previewEnabled() && previewOpen() ? "main-content--preview" : ""}`}>
      ...
      <Show when={active() === "telegram"} fallback={
        ...
        <section class="telegram-panel agent-panel">
          ...
          <div class="settings-card"><div><strong>Remote commands</strong><span>Full agent control without returning to the desktop.</span></div><div class="agent-command-grid"><code>/status</code><span>Session, queue, model, project, and harness</span><code>/model</code><span>Choose a model with buttons</span><code>/project</code><span>Choose a project or Scratch</span><code>/queue</code><span>Inspect queued work</span><code>/steer</code><span>Redirect the active task</span><code>/interrupt</code><span>Stop and replace active work</span><code>/history</code><span>Recent persistent turns</span><code>/schedules</code><span>Inspect scheduled work</span><code>/new</code><span>Start a clean session</span></div></div>
