...
export const DEFAULT_VISIBLE_CONTEXT_BUDGET = 9_000
export const MAX_CHECKPOINT_CHARS = 3_000

export function visibleConversationContext(items: ContextMessage[], summary?: string, budget = DEFAULT_VISIBLE_CONTEXT_BUDGET): string {
...
export function checkpointFor(items: ContextMessage[]): string | undefined {
  ...
  return items.slice(-20).map((message) => message.logs.filter((log) => log.kind === "text").map((log) => log.content.replace(/<app_action>[\s\S]*?<\/app_action>/g, "")).join(" ").replace(/\s+/g, " ").trim()).filter(Boolean).join(" ").slice(-MAX_CHECKPOINT_CHARS)
}
